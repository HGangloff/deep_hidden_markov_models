'''
Code to use, on a single image and multi class image,
the models from Generalized Pairwise and Triplet Markov Chains: example of a
...
def seg_image(nb_classes, img_path, img_name, key, add_noise=True):
    np.seterr(all="warn")
    ...
    if add_noise:
        # we provide a ground truth image that we corrupt by a simulated noise
        if img_name == "cattle3":
            r, rr = 0.75, 1.5
        ...
        # we provide directly the observations and do not have ground truth
...
if __name__ == "__main__":
    ...
    img_name = 'cattle3'
    #img_path = './test_images/leonberger_200.png'
    #img_name = 'leonberger_200'
    nb_classes = 3
    seg_image(nb_classes, img_path, img_name, key, add_noise=True)
